...
3. **Share the Canvas URL** with the user. The URL is returned by \`canvas_create_doc\` and has the format \`${API_URL}/doc/{id}\`. This is the collaborative version where the team can comment, edit together in real-time, and use the AI chat sidebar.
...
1. Call \`canvas_read_doc\` with the document ID to get the latest markdown content from Canvas.
...
server.tool(
  "canvas_list_docs",
  "List all documents in Canvas. Returns each document's ID, title, timestamps, and collaborative URL. The URL format is always {CANVAS_API_URL}/doc/{id}.",
  {},
  async () => {
    const data = await api("/api/canvas/docs");
    const docs = (data.docs || []).map((doc: Record<string, unknown>) => ({
      ...doc,
      url: `${API_URL}/doc/${doc.name}`,
    }));
    return {
...
server.tool(
  "canvas_create_doc",
  "Create a new rich-text document in Canvas. Content should be markdown — headings, bold, italic, code blocks, lists, and blockquotes will all render properly in the editor. Optionally link to a GitHub repo and file path so the doc is born connected — enabling push-to-PR via canvas_push_doc. Returns the new document's ID and collaborative URL (format: {CANVAS_API_URL}/doc/{id}). IMPORTANT: The URL path is /doc/ not /document/.",
  {
  ...
  async ({ title, content, name, repo, filePath }) => {
    ...
    const lines = [
      `Created document "${title}" with ID: ${data.name}`,
      `URL: ${API_URL}/doc/${data.name}`,
    ];
    if (repo && filePath) {
...
server.tool(
  ...
  async ({ docId }) => {
    const data = await api(`/api/canvas/docs/${docId}/github-push`, {
      method: "POST",
    });
    if (data.error) {
    ...
    if (data.commitSha) lines.push(`Commit: ${data.commitSha}`);
...
server.tool(
  ...
  async ({ docId }) => {
    const [meta, body] = await Promise.all([
      api(`/api/canvas/docs/${docId}`),
      api(`/api/canvas/docs/${docId}/content`),
    ]);
    if (meta.error) {
...
server.tool(
  ...
  async ({ docId, title }) => {
    await api(`/api/canvas/docs/${docId}`, {
      method: "PUT",
      body: JSON.stringify({ title }),
    });
    return {
      content: [
        { type: "text", text: `Updated title of ${docId} to "${title}"` },
      ],
    };
  }
);
...
server.tool(
  ...
  async ({ docId, content }) => {
    await api(`/api/canvas/docs/${docId}/content`, {
      method: "PUT",
      body: JSON.stringify({ content }),
    });
    return {
      content: [
        { type: "text", text: `Wrote ${content.length} chars to document ${docId}\nURL: ${API_URL}/doc/${docId}` },
      ],
    };
  }
);
...
server.tool(
  ...
  async ({ docId }) => {
    await api(`/api/canvas/docs/${docId}`, { method: "DELETE" });
    return {
      content: [{ type: "text", text: `Deleted document ${docId}` }],
    };
  }
);
...
server.tool(
  ...
  async ({ docId }) => {
    const data = await api(`/api/canvas/docs/${docId}/comments`);
    return {
      content: [
        {
          type: "text",
...
server.tool(
  ...
  async ({ docId, text, userName, isAgent }) => {
    await api(`/api/canvas/docs/${docId}/comments`, {
      method: "POST",
      body: JSON.stringify({
        text,
    ...
    return {
      content: [
        { type: "text", text: `Added comment to document ${docId}` },
      ],
    };
  }
);
